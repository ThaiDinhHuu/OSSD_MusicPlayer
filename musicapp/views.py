...
def index(request):
    ...
    # Display vietnam Songs
    songs_vietnam = list(Song.objects.filter(language='Vietnamese').values('id'))
    sliced_ids = [each['id'] for each in songs_vietnam][:5]
    indexpage_vietnam_songs = Song.objects.filter(id__in=sliced_ids)
    ...
    context = {
        ...
        'vietnam_songs':indexpage_vietnam_songs,
...
def vietnam_songs(request):

    vietnam_songs = Song.objects.filter(language='Vietnamese')
    ...
    if query:
        vietnam_songs = Song.objects.filter(Q(name__icontains=query)).distinct()
        context = {'vietnam_songs': vietnam_songs}
        return render(request, 'musicapp/vietnam_songs.html', context)

    context = {'vietnam_songs':vietnam_songs,'last_played':last_played_song}
    return render(request, 'musicapp/vietnam_songs.html',context=context)
...
def google_login(request):
    ...
    return render(request, 'musicapp/google_login.html')
